...
- **A standalone "Data volumes" panel** — one persistent control for how much of each entity you want, showing *live* (seeded) / *plan* (generated) / *target* per row. Raising a target **adds** that many on the next run; it never rewrites or removes what's there (that's `prebuild`'s incremental default). You tune it here, not inside the run flow.
- **A guided "Set up demo data" flow** — now two steps: **Options** (all layers or stop at one, export CSV, "start data fresh") → **Review & run**, showing the exact equivalent command, then **Preview (dry run)** and only after that **Run for real**. Amounts come from the Data volumes panel; preview and live regenerate `data/*.json` the same way, so they always agree.

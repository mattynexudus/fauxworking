...
| **Multiple locations** | ❌ Not applicable | The connected Nexudus MCP account has exactly **one** business — there's nothing to distribute across. `business_id` is used as a single scalar everywhere in the generators. Revisit if a multi-location instance is ever targeted. |
